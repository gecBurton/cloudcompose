...
// TestMain_LogsRequiresEnv confirms `cloud-compose logs` has no default
...
func TestLogs_RejectsGcpBeforeParsingCompose(t *testing.T) {
	...
	cmd := exec.Command(bin, "logs", "-f", invalidComposeFile, "-e", envFile)
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("expected cloud-compose logs to fail for a gcp environment, got:\n%s", out)
	}
