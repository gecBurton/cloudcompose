...
// TestMain_PsRequiresEnv confirms `cloud-compose ps` has no default
...
func TestPs_RejectsGcpBeforeParsingCompose(t *testing.T) {
	...
	cmd := exec.Command(bin, "ps", "-f", invalidComposeFile, "-e", envFile)
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("expected cloud-compose ps to fail for a gcp environment, got:\n%s", out)
	}
