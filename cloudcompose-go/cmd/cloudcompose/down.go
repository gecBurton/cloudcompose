...
// app's own Terraform directory (app-<environment name>), the inverse
// of `cloudcompose compile`. It deliberately never touches the shared
// environment `cloudcompose init` created -- destroying a VPC/ALB/ECS
// cluster out from under every other app still using it is not
// something a single app's teardown should ever do implicitly. Tearing
// down the environment itself is a separate, explicit
// `terraform destroy` run by hand in its own env-<name> directory (see
// docs/authored-environment-config.md), matching the same "environment
// is shared, apps are not" split `cloudcompose init`/`compile` already
// draw.
//
...
var downCmd = &cobra.Command{
	...
	Long: "Runs `terraform destroy` in the app's own Terraform directory " +
		"(app-<environment name>, written by a previous `cloudcompose " +
		"compile`), the inverse of `compile`.\n\n" +
