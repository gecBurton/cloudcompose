...
func init() {
	...
	envUpCmd.Flags().StringP("env", "e", "environment.yaml", "Path to the authored environment.yaml (see docs/environment-and-state.md). up itself applies (writes main.tf.json for, then terraform applies) this file -- everywhere else, --env means the same file, but already applied.")
