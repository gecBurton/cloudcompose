...
// TestCalculateListenerPriorities_PinnedBandValue pins the priority band
// for "hello" to a fixed regression value (11401) -- the hash-based band
// derivation must produce the same output on every run, or listener rule
// priorities silently collide across a fleet compiled at different times.
func TestCalculateListenerPriorities_PinnedBandValue(t *testing.T) {
	t.Parallel()
	app := &models.Application{
		Name: "hello",
		Services: []models.Service{
	...
	if priorities["web"] != 11401 {
		t.Errorf("priorities[web] = %d, want 11401 (pinned regression value for _priority_band('hello'))", priorities["web"])
	}
}

// TestCalculateListenerPriorities_MoreSpecificPathsEvaluatedFirst checks
// that within one application, a longer (more specific) path must get a
// lower (higher-precedence) priority number than a shorter one, so /api is
// matched before /.
...
// TestCalculateListenerPriorities_TwoApplicationsDoNotCollide checks that
...
// TestPriorityBand_StableAndBounded checks that determinism is a stated
// guarantee, so the band cannot come from a salted hash (Go map/string
// hashing is randomized per-process unless a fixed hash like sha256 is used
...
// TestPathPatterns_AllCases fills in the /api (no trailing slash) and /a/b
// cases that the existing TestPathPatterns test did not.
func TestPathPatterns_AllCases(t *testing.T) {
	t.Parallel()
	cases := []struct {
		path string
		want []string
...
// TestInferNetworking_NoExplicitNetworksIsFlat checks that compose
// materializes a single "default" network when none is declared, so
// everything can reach everything -- the behavior of a compose file that
// says nothing. Uses the real scaling example, which declares no
// networks: block, through the real parser/normalizer boundary.
