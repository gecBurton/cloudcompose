...
// an application with no resources at all: just the aws provider,
// required_providers, and an *empty but present* resource block. The
// Resource field is never nil (see NewAWSResources' default construction),
// so the output always has "resource": {} rather than omitting the key.
...
// repeatedly and diffs the output byte-for-byte: every new map-shaped
// output gets this check as it's written, not discovered later the way
// an earlier ordering bug was.
...
// round-tripping) is stripped from the output. Constructed via
// strings.Contains rather than full unmarshalling since the point under
// test is precisely which keys survive serialization.
