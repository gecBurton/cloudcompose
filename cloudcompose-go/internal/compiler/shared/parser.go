...
func ParseCompose(filePath string) (*models.ComposeApplication, error) {
	...
	// must not be empty") where v1 left it for cloudcompose's own Normalize to
	...
	// does. That path needs to be re-rooted, for exactly this reason: an
	// absolute path here leaks the local filesystem into the generated
	// Terraform (docker_image.build.context) and makes output depend on
	// where the repository happens to be checked out, rather than
	// compiling to the same thing on any machine.
