...
// buildComposeyBinary builds the composey CLI once per test run and
...
func buildComposeyBinary(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "composey")
	cmd := exec.Command("go", "build", "-o", bin, ".")
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("build composey: %v\n%s", err, out)
	}
	return bin
}
