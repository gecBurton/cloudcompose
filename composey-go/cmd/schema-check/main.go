...
// composey generates config for (via `terraform providers schema -json`,
...
// that composey actually marshals into that JSON.
//
...
// during the idiomatic-Go cleanup pass (see plan.md) -- this tool exists
// so the next one is caught by running it, not by manually re-reading
// provider docs field by field.
//
// Usage:
//
...
// providerSpec is one entry in the required_providers block composey's
...
func main() {
	...
	schemaDir, err := os.MkdirTemp("", "composey-schema-check-*")
	if err != nil {
...
// in a scratch directory declaring every provider composey generates
...
// is a dev-time check, not something run as part of `composey`'s own
// compile path.
...
// every provider source listed, since composey's own field-to-resource
...
// (and composey should therefore represent as) a bare object rather than
...
// map[string]SomeStruct type -- composey's own convention throughout
...
func checkResourceSet(set resourceSet, schema map[string]any) (mismatches, unclassified int) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// composey's own convention: every resource-type field is
