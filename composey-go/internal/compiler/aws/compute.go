...
func handlePlatformConfig(
	...
	// own order. Key order in the resulting JSON string has no observable
	// effect (Secrets Manager stores it as an opaque blob until read back
	// out by key), but service.Config's own order is preserved anyway
	// since that's the natural iteration order here.
