...
// mirroring _connectivity.py's _sg_key.
...
// network segments, mirroring _connectivity.py's security_group_ids.
...
// IsDiscoverable checks if a service can be discovered by other services,
// mirroring _connectivity.py's _is_discoverable. A scheduled task runs and
// exits rather than being a service, and a container with no port publishes
// nothing to reach.
...
// application, mirroring _connectivity.py's infer_networking. Creates one
// security group per network isolation segment. Services sharing a segment
// can reach each other, and services on disjoint segments cannot. This maps
// to AWS security groups; other clouds use equivalent mechanisms.
...
// services, mirroring _connectivity.py's infer_service_discovery. Returns
// the namespace name for use in connection strings.
