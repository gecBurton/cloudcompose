...
func TestInferAWS_GoldenExamplesByteIdentical(t *testing.T) {
	for _, name := range awsGoldenExamples {
		...
		t.Run(name, func(t *testing.T) {
			...
			// goal now that nothing needs to match Python's specific
			// json.dumps spacing anymore -- only that the two sides
			// describe the same JSON value once fully parsed, at every
			// level, embedded strings included.
