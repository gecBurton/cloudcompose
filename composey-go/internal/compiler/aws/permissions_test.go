...
// mirroring _permissions.py's _grant_s3_permissions using client_task_role.
...
// TestInferPermissionsAndWiring_RelationshipAloneGrantsNothing mirrors
// test_permissions.py's test_depends_on_alone_grants_nothing: a
// Relationship with no actual env-var reference to the server must not
...
// mirrors test_permissions.py's
// test_a_grant_is_scoped_to_the_service_actually_referenced: with two
// object-storage services, only the one actually named in an env var earns
// an S3 policy; a Relationship-only sibling earns nothing.
