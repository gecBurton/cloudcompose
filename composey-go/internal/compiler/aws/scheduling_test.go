...
// TestEventbridgeExpression_MatchesPython pins each case against a live
// run of Python's _eventbridge_expression (2026-08-06), not just this
// port's own idea of what it should produce.
func TestEventbridgeExpression_MatchesPython(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name     string
...
// TestEventbridgeExpression_RejectsBothDayFieldsConstrained matches
// Python's ScheduleError for a cron expression constraining both
// day-of-month and day-of-week, which EventBridge cannot express.
...
// caught as a real divergence when a scratch end-to-end run against the
// production-stack example (which has a cron schedule) panicked with
// "unknown schedule type *models.CronSchedule" despite every hand-built
// value-type test passing (2026-08-06).
