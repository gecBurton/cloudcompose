...
// InferAzure is the main entry point for Azure inference, mirroring
// compiler/inference/azure/__init__.py's infer().
func InferAzure(app *models.Application, env *models.AzureEnvironment) (*models.AzureResources, error) {
	...
	// order they were inserted (a Python dict preserves insertion order;
	// Go's map does not), and that order determines which _URL env var
	// comes first when a service references more than one. Confirmed as
	// a real, not theoretical, divergence by diffing actual output for
...
// connectionOrderForAzure returns connection keys in the order Python's
// own dict-merge (connections from _infer_databases, then
// .update(cache_connections), then .update(storage_connections) -- each
// built by iterating app.services in declaration order within its own
// capability filter) would produce: every database-capability service
// (in declaration order), then every cache-capability service, then every
// object-storage-capability service, filtered to those with a connection.
...
// service builds from source, mirroring _infer_container_registry.
func inferContainerRegistry(resources *models.AzureResources, app *models.Application, env *models.AzureEnvironment, tags map[string]string) {
	...
	for i := range app.Services {
		...
		// PyOrdered, not a plain map: Python's dict literal here is
		// {"context":..., "platform":...} with "dockerfile" appended
		// last, and Azure's generator has no sort_keys=True to fall back
		// on (unlike AWS's), so key order in this specific dict survives
		// into the final Terraform JSON byte-for-byte -- confirmed as a
		// real divergence by diffing actual output against the flask/
		// build-webapp/doctor golden files (2026-08-06), not assumed.
		build := PyOrdered{
			p("context", *service.BuildContext),
			p("platform", "linux/amd64"),
		}
		if service.Dockerfile != nil {
			build = append(build, p("dockerfile", *service.Dockerfile))
		}
