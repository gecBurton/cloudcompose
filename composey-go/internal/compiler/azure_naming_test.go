...
// TestAzureNaming_MatchesPython pins every naming function's output
// against a live run of the Python implementation (2026-08-06), not just
// this port's own idea of what the transliteration should produce -- the
// AWS port's own review discipline applied here too.
func TestAzureNaming_MatchesPython(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		got  string
