...
// InferComputeResources infers ECS Fargate compute resources, mirroring
// _compute.py's infer_compute_resources.
//
...
func InferComputeResources(
	...
	for i := range app.Services {
		...
		// Get compute sizing.
		compute, ok := SizeMappings[string(service.Size)]
		if !ok {
			compute = SizeMappings["small"]
		}
		cpu := compute.CPU
		...
		// Create container definition. Built directly as PyOrdered rather
		// than through models.ContainerDefinition + encoding/json: Pydantic's
		// model_dump(exclude_none=True) both preserves field declaration
		// order (name, image, essential, command, portMappings, environment,
		// secrets, logConfiguration) and omits None fields entirely, and
		// PyOrdered is what makes both of those hold exactly rather than
		// coincidentally through struct tags.
		var portMappings []any
		if service.Port != nil {
			portMappings = []any{
				PyOrdered{
					p("containerPort", *service.Port),
					p("hostPort", *service.Port),
					p("protocol", "tcp"),
				},
			}
		} else {
		...
		environment := make([]any, 0, len(service.Env))
		for _, k := range sortedKeys(service.Env) {
			environment = append(environment, PyOrdered{p("name", k), p("value", service.Env[k])})
		}

		secretsList := make([]any, 0, len(containerSecrets))
		for _, s := range containerSecrets {
			secretsList = append(secretsList, PyOrdered{p("name", s["name"]), p("valueFrom", s["valueFrom"])})
		}

		containerFields := PyOrdered{
			p("name", service.Name),
			p("image", containerImage),
			p("essential", true),
		}
		if len(service.Command) > 0 {
			containerFields = append(containerFields, p("command", service.Command))
		}
		containerFields = append(containerFields,
			p("portMappings", portMappings),
			p("environment", environment),
			p("secrets", secretsList),
			p("logConfiguration", PyOrdered{
				p("logDriver", "awslogs"),
				p("options", PyOrdered{
					p("awslogs-group", fmt.Sprintf("${aws_cloudwatch_log_group.%s.name}", logGroupKey)),
					p("awslogs-region", env.Region),
					p("awslogs-stream-prefix", AWSLogsStreamPrefix),
				}),
			}),
		)

		containerJSON := PyDumps([]PyOrdered{containerFields})
...
// createIamRoles creates IAM roles for ECS task and execution, mirroring
// _compute.py's _create_iam_roles.
func createIamRoles(
	resources *models.AWSResources,
...
func ecsTasksAssumeRolePolicy() string {
	return PyDumps(PyOrdered{
		p("Version", IAMPolicyVersion),
		p("Statement", []PyOrdered{
			{
				p("Action", "sts:AssumeRole"),
				p("Effect", "Allow"),
				p("Principal", PyOrdered{p("Service", "ecs-tasks.amazonaws.com")}),
			},
		}),
	})
}
...
// mirroring _compute.py's _create_log_policy.
func createLogPolicy(
	...
	policy := PyDumps(PyOrdered{
		...
		p("Statement", []PyOrdered{
			{
				p("Effect", "Allow"),
				p("Action", []string{"logs:CreateLogStream", "logs:PutLogEvents"}),
				p("Resource", []string{
...
// handleBuildContext handles build-from-source services, mirroring
// _compute.py's _handle_build_context.
...
// resources, mirroring _compute.py's _handle_secrets.
func handleSecrets(
	resources *models.AWSResources,
	service *models.Service,
	app *models.Application,
	...
	for _, secretName := range service.Secrets {
		...
		resources.SecretsmanagerSecretVersion[secretKey+"_v1"] = models.SecretsManagerSecretVersion{
			...
			SecretString: SecretsPlaceholderValue,
		...
		policy := PyDumps(PyOrdered{
			...
			p("Statement", []PyOrdered{
				{
					p("Effect", "Allow"),
					p("Action", []string{"secretsmanager:GetSecretValue"}),
					p("Resource", []string{fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}", secretKey)}),
				},
			}),
		})
...
// not valued in the compose file), mirroring _compute.py's
...
func handlePlatformConfig(
	...
	// Python: {key: SECRETS_PLACEHOLDER_VALUE for key in service.config} --
	// a dict comprehension over the list, which preserves service.config's
	// own order (not sorted), so this has to too.
	placeholders := make(PyOrdered, 0, len(service.Config))
	for _, key := range service.Config {
		placeholders = append(placeholders, p(key, SecretsPlaceholderValue))
	}
	secretString := PyDumps(placeholders)
		...
		p("Statement", []PyOrdered{
			{
				p("Effect", "Allow"),
				p("Action", []string{"secretsmanager:GetSecretValue"}),
				p("Resource", []string{fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}", configKey)}),
			},
		}),
	})
...
// createEcsService creates the ECS service configuration, mirroring
// _compute.py's _create_ecs_service.
...
// handleIngress handles public ingress configuration (ALB), mirroring
// _compute.py's _handle_ingress.
func handleIngress(
	resources *models.AWSResources,
	service *models.Service,
	env *models.AwsEnvironment,
...
// handleAutoscaling handles auto-scaling configuration, mirroring
// _compute.py's _handle_autoscaling. Supports configurable metrics (CPU,
// memory, requests per target) with customizable target values and
// cooldown periods.
func handleAutoscaling(
	...
	//
	// Python: `config = service.auto_scaling or AutoScalingConfig()`, and
	// AutoScalingConfig's own Pydantic defaults are not an empty
	// configuration: default_factory supplies CPU 70%/Memory 80% metrics
	// and 300s/60s cooldowns. A bare zero-value models.AutoScalingConfig{}
	// in Go has none of that -- confirmed as a real, silent divergence (not
	...
	for i, metric := range config.Metrics {
		...
		policyConfig := map[string]any{
			...
			"target_value":                    PyFloat(metric.TargetValue),
...
// defaultAutoScalingConfig mirrors AutoScalingConfig's own Pydantic
// defaults in semantic.py: CPU 70%, Memory 80%, 300s scale-in cooldown, 60s
// scale-out cooldown. Used whenever a scaling service declares no explicit
// auto_scaling block, matching Python's `service.auto_scaling or
// AutoScalingConfig()`.
func defaultAutoScalingConfig() *models.AutoScalingConfig {
	return &models.AutoScalingConfig{
		Metrics: []models.AutoScalingMetric{
			{Type: models.AutoScalingMetricCPU, TargetValue: AutoScalingCPUTarget},
			{Type: models.AutoScalingMetricMemory, TargetValue: AutoScalingMemoryTarget},
		},
		ScaleInCooldown:  300,
		ScaleOutCooldown: 60,
	}
