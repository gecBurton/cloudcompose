...
func TestInferEdgeResources_RealProductionStackExample(t *testing.T) {
	t.Parallel()
	composeApp, err := ParseCompose("../../../examples/production-stack/compose.yml")
	if err != nil {
		t.Fatalf("ParseCompose failed: %v", err)
	}
	app, err := Normalize(composeApp, "production-stack")
	if err != nil {
	...
	if waf.Provider == nil || *waf.Provider != CloudFrontProviderRef {
		t.Errorf("Provider = %v, want %q", waf.Provider, CloudFrontProviderRef)
	}
