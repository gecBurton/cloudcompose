...
// rendered output against a live Python run (captured 2026-08-06), the
// same discipline as the AWS/Azure ports: byte-identical, not merely
// equivalent.
...
// TestExplain_MultiPortWarnsAboutIgnoredPorts mirrors _port_decisions'
// warning branch, exercised through the real composeApp (not nil) path
// -- the branch every current caller in cli.py never actually reaches
// (both call sites pass docker_app=None, since the Python parser that
// could produce one was removed in Phase 2), but ported for fidelity per
// this phase's own decision, and verified working here rather than left
// untested because nothing calls it yet.
...
// TestExplain_DroppedMountsWarning mirrors _volume_decisions, also only
// reachable through the composeApp-provided path.
...
// TestExplain_NoPublicServiceWarnsApplicationUnreachable mirrors
// _ingress_decisions' fallback branch.
...
// TestExplain_CannotReachWarning mirrors _wiring_decisions' "cannot
...
// TestExplain_NothingWiredWarning mirrors _wiring_decisions' "nothing
...
// --- Additional tests mirroring test_explain.py exactly, using synthetic
// ComposeApplication/Application pairs the way that file's decisions_for()
// helper builds both by hand (restored after the Go port deleted
// parser.py/normalizer.py; explain() takes both models directly and does
// not depend on Normalize() to produce them). ---

func intPtrExplain(i int) *int { return &i }

// TestExplain_WiredManagedServiceIsNotAWarning mirrors
// test_wired_managed_service_is_not_a_warning: a correctly-wired
// reference produces the "KEY -> server" decision and no "nothing
// wired" warning.
...
// TestExplain_EmptySecretsAreWarnedAbout mirrors
// test_empty_secrets_are_warned_about.
...
// TestExplain_SubstitutionReportsTheImageItMatched mirrors
// test_substitution_reports_the_image_it_matched: an inferred (not
// declared) capability substitution reports the matched image in
// Because, with Source == SourceInferred.
...
// TestExplain_BuildReportsTheDockerfile mirrors
// test_build_reports_the_dockerfile.
...
// TestExplain_RenderCountsWarnings_WithWarningsPresent mirrors
// test_render_counts_warnings, specifically the "worth checking" branch
// (only the zero-warnings branch was exercised via the hello golden
// fixture before this test).
