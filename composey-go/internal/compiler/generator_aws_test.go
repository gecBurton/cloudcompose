...
func TestGenerateAWS_CloudfrontScopedWAFPinsProvider(t *testing.T) {
	...
	if edge["region"] != CloudFrontScopeRegion {
		t.Errorf("expected edge provider region %s, got %v", CloudFrontScopeRegion, edge["region"])
	}
	if edge["alias"] != CloudFrontProviderAlias {
		t.Errorf("expected edge provider alias %s, got %v", CloudFrontProviderAlias, edge["alias"])
	}
}
