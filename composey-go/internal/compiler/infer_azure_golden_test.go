...
// TestGenerateAzure_ByteIdentical additionally checks the raw byte output
// (not just structural JSON equality) for one example, since Azure's
// generator has no sort_keys equivalent at all -- unlike AWS, where
// structural equality was a deliberately weaker check than a byte diff
// because Python's own key order didn't matter for AWS's sort_keys=True
// output. Here it does, at every level, so this pins that directly.
func TestGenerateAzure_ByteIdentical(t *testing.T) {
	t.Parallel()
	...
	expected, err := os.ReadFile("../../../examples/hello/expected/azure/main.tf.json")
	if err != nil {
		t.Fatalf("read golden file: %v", err)
	}
	if actual != string(expected) {
		t.Errorf("byte-level mismatch.\n--- got ---\n%s\n--- want ---\n%s", actual, string(expected))
	}
}
...
// TestInferContainerRegistry_BuildDictKeyOrderMatchesPython pins the exact
// key order of the docker_image build dict: context, platform, then
// dockerfile last -- caught as a real divergence, not a theoretical one,
// when this phase's byte-identical-output check against the flask/
// build-webapp/doctor golden files surfaced a plain map[string]any
// sorting these alphabetically (context, dockerfile, platform) instead
// (2026-08-06).
func TestInferContainerRegistry_BuildDictKeyOrderMatchesPython(t *testing.T) {
	...
	build, ok := image.Build.(PyOrdered)
	if !ok {
		t.Fatalf("expected image.Build to be PyOrdered, got %T", image.Build)
	}
	if len(build) != 3 {
		t.Fatalf("expected 3 build keys, got %d", len(build))
	}
	want := []string{"context", "platform", "dockerfile"}
	for i, pair := range build {
		if pair.Key != want[i] {
			t.Errorf("build key[%d] = %q, want %q", i, pair.Key, want[i])
		}
	}
}
...
func TestContainerSpecAzure_MissingConnectionFieldsRenderAsNone(t *testing.T) {
	...
	spec := containerSpecAzure(&app.Services[0], app, &env, connections, []string{"blobs"})
	var envVars []any
	for _, pair := range spec {
		if pair.Key == "env" {
			envVars = pair.Value.([]any)
		}
	}
	if len(envVars) != 1 {
		t.Fatalf("expected 1 env var, got %d", len(envVars))
	}
	entry := envVars[0].(PyOrdered)
	var value string
