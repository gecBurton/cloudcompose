...
// returns their connections, mirroring _managed.py's infer_managed_services.
//
...
func InferManagedServices(
	...
	_ = namespace // parity with Python, which computes but does not use it here either
...
// inferDatabase infers RDS database resources, mirroring _managed.py's
// _infer_database.
func inferDatabase(
	resources *models.AWSResources,
	...
	dbUsername := DatabaseDefaultUsername
	...
	secretString := PyDumps(PyOrdered{
		p("username", dbUsername),
		p("password", fmt.Sprintf("${random_password.%s.result}", passwordKey)),
		p("engine", engine),
	})
	...
	instanceClass, ok := DBInstanceClasses[string(service.Size)]
	if !ok {
		instanceClass = DBInstanceClasses["small"]
	}
	...
	port := DefaultPortForDatabase(engine)
...
// inferCache infers ElastiCache Redis resources, mirroring _managed.py's
// _infer_cache.
func inferCache(
	resources *models.AWSResources,
	service *models.Service,
	...
	nodeType, ok := CacheNodeTypes[string(service.Size)]
	if !ok {
		nodeType = CacheNodeTypes["small"]
	}
...
// mirroring _managed.py's _infer_object_storage.
