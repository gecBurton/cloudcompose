...
func TestInferManagedServices_RealNginxFlaskMysqlExample(t *testing.T) {
	t.Parallel()
	composeApp, err := ParseCompose("../../../examples/nginx-flask-mysql/compose.yml")
	if err != nil {
		t.Fatalf("ParseCompose failed: %v", err)
	}
	app, err := Normalize(composeApp, "nginx-flask-mysql")
	if err != nil {
	...
	if dbInstance.Username == nil || *dbInstance.Username != "composey" {
		t.Errorf("Username = %v, want composey", dbInstance.Username)
	}
...
func TestInferManagedServices_RealFlaskRedisExample(t *testing.T) {
	t.Parallel()
	composeApp, err := ParseCompose("../../../examples/flask-redis/compose.yml")
	if err != nil {
		t.Fatalf("ParseCompose failed: %v", err)
	}
	app, err := Normalize(composeApp, "flask-redis")
	if err != nil {
		t.Fatalf("Normalize failed: %v", err)
	}
...
func TestInferManagedServices_RealMinioS3Example(t *testing.T) {
	t.Parallel()
	composeApp, err := ParseCompose("../../../examples/minio-s3/compose.yml")
	if err != nil {
		t.Fatalf("ParseCompose failed: %v", err)
	}
	app, err := Normalize(composeApp, "minio-s3")
	if err != nil {
		t.Fatalf("Normalize failed: %v", err)
	}
...
// TestInferObjectStorage_DiscardForcesDestroy mirrors test_data_retention.py's
// test_a_throwaway_environment_discards_everything (S3 half): a non-retained
...
// TestInferDatabase_SecretsAlwaysHardDeletedRegardlessOfRetention mirrors
// test_data_retention.py's test_secrets_are_always_hard_deleted: unlike the
// database/bucket, a credentials secret's recovery_window_in_days is 0
// regardless of retain_data_on_destroy -- a recovery window keeps the name
// reserved, which blocks re-creating one with the same name for up to 30
// days, and a retained database is recoverable from its own snapshot
// without the old credentials anyway. Checked for both discard=true and
// discard=false, since only the discard=true path had any coverage before
// (via the general discard test) and the "regardless of" half specifically
// needs both.
