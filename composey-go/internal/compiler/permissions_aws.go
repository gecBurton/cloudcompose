	...
	for i := range app.Services {
		...
		for _, e := range rawEnv {
			...
			if !resolved.Confidential {
				environment = append(environment, PyOrdered{p("name", name), p("value", resolved.Value)})
				continue
			}
		...
		// Re-render through PyDumps-equivalent ordering is unnecessary here:
		// container is a map[string]any decoded from the original
		// PyDumps-rendered JSON, so its own key order was lost the moment
		// it went through json.Unmarshal. Rebuilt as PyOrdered below to
		// restore Python's field order rather than accepting
		// encoding/json's alphabetical map ordering.
		taskDef.ContainerDefinitions = rebuildContainerDefinitionsJSON(containerDefs)
		_ = newContainerDefs
...
// rebuildContainerDefinitionsJSON re-renders the (already environment- and
// secrets-mutated) container definitions map back into Python-key-order
// JSON. The map lost its original field order when json.Unmarshal decoded
// it; PyOrdered restores name/image/essential/.../logConfiguration order
// to match what _permissions.py's own json.dumps(container_defs) produces,
// since it mutates the same dict in place rather than rebuilding it.
func rebuildContainerDefinitionsJSON(containerDefs []map[string]any) string {
	ordered := make([]PyOrdered, 0, len(containerDefs))
	for _, c := range containerDefs {
		obj := PyOrdered{}
		for _, key := range []string{"name", "image", "essential", "command", "portMappings", "environment", "secrets", "logConfiguration"} {
			if v, ok := c[key]; ok {
				obj = append(obj, p(key, anyToPyValue(v)))
			}
		}
		ordered = append(ordered, obj)
	}
	return PyDumps(ordered)
}

// anyToPyValue converts a decoded JSON value (map[string]any / []any /
// json.Number / etc., as produced by unmarshalling into `any`) into
// something PyDumps renders correctly: map[string]any becomes PyOrdered
// with keys in the order encoding/json happened to decode them (map
// iteration order is not guaranteed, but every map here originated from
// PyOrdered's own JSON, whose only nested objects -- portMappings entries,
// logConfiguration -- have very few keys where alphabetical and original
// order coincide often enough to not matter for the fields this package
// actually mutates: environment and secrets, both already rebuilt as
// PyOrdered directly rather than passed through this path).
func anyToPyValue(v any) any {
	switch val := v.(type) {
	case []any:
		out := make([]any, len(val))
		for i, item := range val {
			out[i] = anyToPyValue(item)
		}
		return out
	case map[string]any:
		...
		for _, k := range keys {
			obj = append(obj, p(k, anyToPyValue(val[k])))
		}
		return obj
	default:
		return val
...
func storeConfidentialValue(
	...
	secrets = append(secrets, PyOrdered{
		p("name", varName),
		p("valueFrom", fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}", urlKey)),
	})
		...
		p("Statement", []PyOrdered{
			{
				...
				p("Resource", []string{fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}", urlKey)}),
			},
		}),
	})
...
func grantDatabasePermissions(
	...
	secrets = append(secrets,
		PyOrdered{
			p("name", "DB_PASSWORD"),
			p("valueFrom", fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}:password::", dbSecretKey)),
		},
		PyOrdered{
			p("name", "DB_USERNAME"),
			p("valueFrom", fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}:username::", dbSecretKey)),
		},
	)
	container["secrets"] = secrets
		...
		p("Statement", []PyOrdered{
			{
				p("Effect", "Allow"),
				p("Action", []string{"secretsmanager:GetSecretValue"}),
				p("Resource", []string{fmt.Sprintf("${aws_secretsmanager_secret.%s.arn}", dbSecretKey)}),
			},
		}),
	})
