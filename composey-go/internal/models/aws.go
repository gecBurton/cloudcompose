...
// AWS resource models mirroring composey/models/aws.py.
//
// Field names and JSON tags match the Python Pydantic models exactly, since
// both are marshalled straight into Terraform's JSON syntax: whatever key a
...
// the zero value, matching Pydantic's `exclude_none=True` behavior in
// generator.py (e.g. a security group rule legitimately has from_port=0).
...
type EcsService struct {
	...
	// No omitempty: Pydantic's load_balancer field uses
	// default_factory=list rather than Optional[...]=None, so Python's own
	// output always includes "load_balancer": [] for a service with no
	// ingress -- confirmed by diffing real output for the nginx-flask-mysql
	// and compute-tuning examples (2026-08-06), both of which have no
	// public ingress and both of which the golden files show
	// "load_balancer": [] for.
...
// reuses them too (mirroring composey/models/terraform_common.py).
...
// strips any resource-type block whose map ends up empty after that, exactly
// as generator.py does.
