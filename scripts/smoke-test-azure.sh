...
# --- Remote state ------------------------------------------------------------
# State on a CI runner is ephemeral. When STATE_RG is set we use Azure Blob
# Storage backend so a leaked run stays destroyable from any machine.
# NOTE: Azure backend requires storage account setup. For now, local state only.
write_backend() {
  local dir="$1" key="$2"
  # Disabled: Azure backend requires pre-created storage account with proper auth
  # TODO: Re-enable after setting up storage account and container
  [[ -n "$STATE_RG" ]] || return 0
  log "WARNING: Azure remote state not yet implemented, using local state"
  return 0
}

state_hint() {
  [[ -n "$STATE_RG" ]] || return 0
  echo "  State kept in $STATE_RG storage account — destroy later with:"
...
if [[ -z "$CONTAINER_APP_FQDN" ]]; then
  ...
  CONTAINER_APP_FQDN="$(eval "$TF state show 'azurerm_container_app.main' 2>/dev/null | grep 'fqdn' | head -1 | awk '{print \$3}' | tr -d '\"' || true)"
fi
