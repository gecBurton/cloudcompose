...
# <dir of its -f>/app-<environment name>, so the app's compose file is
# first copied into its own build/ subdirectory (APP_BUILD_SRC below)
# purely so that output lands under build/ rather than inside
# examples/ itself.
...
BUILD_DIR="$APP_BUILD_SRC/app-$NAME"                      # generated by 'cloudcompose compile'
