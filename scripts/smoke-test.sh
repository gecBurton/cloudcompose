...
PROJECT="${PROJECT:-hello}"                              # cloud-compose project name
...
cleanup() {
  ...
  if [[ -x "$CLOUDCOMPOSE" && -f "$BUILD_DIR/main.tf.json" ]]; then
    ...
    "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" -p "$PROJECT" --tail 500 || true
  fi
...
show_diagnostics() {
  ...
  "$CLOUDCOMPOSE" compose ps -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" -p "$PROJECT" || true
  log "cloud-compose compose logs, last 5m ($label)…"
  "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" -p "$PROJECT" --since 5m --tail 200 || true
}
...
while (( SECONDS < ps_deadline )); do
  if "$CLOUDCOMPOSE" compose ps -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" -p "$PROJECT" --json | python3 -c "
import json, sys
...
while (( SECONDS < logs_deadline )); do
  if "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" -p "$PROJECT" --since 5m --tail 200 --json | python3 -c "
import json, sys
