...
#
# Creates environment with 'cloud-compose env init', compiles a compose file,
...
PROJECT="${PROJECT:-hello}"                              # cloud-compose project name (written into
                                                         # the compose copy's own `name:` before
                                                         # compiling, see step 2 below)
...
# <dir of its -f>/app-<environment name>-<project name>, where project
# name is the compose file's own `name:`, not a flag -- so the copy
# under APP_BUILD_SRC has its `name:` rewritten to $PROJECT before
# compiling (see step 2 below). ENV_DIR itself is never passed to -e/
# --env on any command -- -e always takes the authored environment.yaml
# ($GENERATED_ENV_CONFIG, set in step 1 below), which every command
# derives ENV_DIR from itself; ENV_DIR is kept here only for this
# script's own cd/destroy/existence-check use.
ENV_CONFIG_DIR="$ROOT/build/$PROVIDER"                    # holds the generated environment.yaml
ENV_DIR="$ENV_CONFIG_DIR/env-$NAME"                       # generated by 'cloud-compose env init'
APP_BUILD_SRC="$ROOT/build/$PROVIDER/$PROJECT-src"        # copy of the compose file's own directory
BUILD_DIR="$APP_BUILD_SRC/app-$NAME-$PROJECT"             # generated by 'cloud-compose compile'
CLOUDCOMPOSE="$ROOT/cloudcompose-go/cloud-compose"
...
# set, the environment.yaml templating step above (search
# "Swap the committed backend:") authors a real `backend.remote:` block
# instead of `local:`, and cloud-compose itself then emits the matching
# `terraform.backend` block into both the environment's own main.tf.json
# and every app compiled against it (see
# internal/compiler/shared/app_backend_block.go), keyed by run NAME so a
# leaked run stays destroyable from any machine. No separate backend
# file is written here -- an earlier version of this script did that on
# top of what env init/compile already emit, which collided with it once
# backend: became mandatory (a real "Duplicate backend configuration"
# failure). Without a configured state variable, cloud-compose's own
# backend: local: {path: ...} is left as authored, which is fine on a
# laptop where the files persist.
...
state_hint() {
  ...
  case "$PROVIDER" in
    aws)
      echo "  State kept at s3://$STATE_BUCKET/cloudcompose/$NAME/ -- destroy later with:"
      ...
      ;;
    azure)
      echo "  State kept at $STATE_ACCOUNT/$STATE_CONTAINER/cloudcompose/$NAME/ -- destroy later with:"
...
purge_state() {
  ...
  az storage blob delete-batch \
    ...
    --pattern "cloudcompose/$NAME/*" \
    --auth-mode login >/dev/null 2>&1 \
    || echo "  NOTE: could not remove state under cloudcompose/$NAME/ -- harmless, prune later."
}
...
cleanup() {
  ...
  # exists, i.e. `cloud-compose compile` itself ran) and $CLOUDCOMPOSE to have
...
# run used. No backend needs writing here -- $ENV_DIR/main.tf.json and
# $BUILD_DIR/main.tf.json already have their real backend.remote block
# baked in from when `env init`/`compile` first created them (see the
# "Remote state" comment above), so a plain `-reconfigure` is all `terraform
# init` needs.
if [[ "${1:-}" == "--destroy-only" ]]; then
  ...
  if [[ -f "$BUILD_DIR/main.tf.json" ]]; then
    ...
    echo "        To destroy it too, first run (project identity comes from the"
    echo "        compose file's own \`name:\`, so it must read \`name: $PROJECT\`):"
    echo "          $CLOUDCOMPOSE compile -f <copy of \$COMPOSE under build/, with name: $PROJECT> -e <the generated environment.yaml next to $ENV_DIR>"
  fi
...
# the moment cloud-compose's source changed underneath it.
log "Building cloud-compose…"
(cd "$ROOT/cloudcompose-go" && go build -tags disable_grpc_modules -o cloud-compose ./cmd/cloudcompose)

# --- 1. Create environment with cloud-compose env init ---------------------------------
log "Creating $PROVIDER environment '$NAME' with cloud-compose env init…"
rm -rf "$ENV_DIR"
cd "$ROOT"
...
# Swap the committed backend: {local: {...}} block for a real remote
# backend when state variables are configured (always true in CI --
# see acceptance.yml/azure-acceptance.yml) -- cloud-compose itself then
# emits the correct backend block for both the environment and every
# app compiled against it, with a deterministic key derived from
# name: (BackendKeyForEnvironment/BackendKeyForApp), so no separate,
# hand-rolled backend file is needed on top of what env init/compile
# already write. Left as backend: {local: {...}} for a bare local run
# (state variables unset).
...
if '$PROVIDER' == 'azure':
    if '$STATE_RG':
        ...
        content = content.replace(local_backend, remote_backend)
else:
    if '$STATE_BUCKET':
        remote_backend = (
            'backend:\n  remote:\n'
        ...
        if '$STATE_TABLE':
            remote_backend += '    dynamodb_table: $STATE_TABLE\n'
        content = content.replace(local_backend, remote_backend)

with open('$GENERATED_ENV_CONFIG', 'w') as f:
    f.write(content)
"
"$CLOUDCOMPOSE" env init -e "$GENERATED_ENV_CONFIG"
...
# Project identity comes from the compose file's own `name:`, not a
# flag. $PROJECT is sometimes shorter than the example's committed
# `name:` (e.g. build-webapp -> buildweb/buildwebapp, to fit Azure's
...
"$CLOUDCOMPOSE" compile -f "$COMPOSE_BUILD_COPY" -e "$GENERATED_ENV_CONFIG"
