...
PROJECT="${PROJECT:-hello}"                              # cloud-compose project name (written into the
                                                         # compose copy's own `name:` field before
                                                         # compiling -- see "2. Compile the app" below)
...
# name is the compose file's own top-level `name:` field, not a flag
# (see docs/deployment-identity-design.md) -- so for BUILD_DIR to match
# what compile actually derives, the compose file's copy under
# APP_BUILD_SRC (below) must itself declare `name: $PROJECT` before
# compile ever runs; see the sed rewrite in "2. Compile the app" below.
...
write_backend() {
  ...
  case "$PROVIDER" in
    aws)
      ...
      # aws.GenerateAwsEnvironment's own choice for real cloud-compose
...
cleanup() {
  ...
  if [[ -x "$CLOUDCOMPOSE" && -f "$BUILD_DIR/main.tf.json" ]]; then
    ...
    "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" --tail 500 || true
  fi
...
if [[ "${1:-}" == "--destroy-only" ]]; then
  ...
  if [[ -f "$BUILD_DIR/main.tf.json" ]]; then
    ...
    echo "        To destroy it too, first run (project identity is the compose"
    echo "        file's own top-level \`name:\`, not a flag -- see"
    echo "        docs/deployment-identity-design.md; it must read \`name: $PROJECT\`):"
    echo "          $CLOUDCOMPOSE compile -f <copy of \$COMPOSE under build/, with name: $PROJECT> -e $ENV_DIR"
  fi
...
# Project identity now comes solely from the compose file's own
# top-level `name:` (see docs/deployment-identity-design.md) -- there is
# no -p/--project flag any more. $PROJECT here is deliberately shorter
# than some examples' own committed `name:` (e.g. build-webapp ->
# buildweb/buildwebapp) to stay under Azure's resource-name length
# limits once combined with $NAME (see azure/naming.go), so rather than
# relying on the committed name, the copy's own `name:` line is
# rewritten to $PROJECT before compiling.
sed -i.bak "s/^name: .*/name: $PROJECT/" "$COMPOSE_BUILD_COPY" && rm -f "$COMPOSE_BUILD_COPY.bak"
COMPILE_ARGS=(-f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR")
if [[ "$PROVIDER" == "azure" ]]; then
  # --subnet-index is required on Azure (no default -- see
  # docs/deployment-identity-design.md); 0 is correct here since exactly
...
show_diagnostics() {
  ...
  "$CLOUDCOMPOSE" compose ps -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" || true
  log "cloud-compose compose logs, last 5m ($label)…"
  "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" --since 5m --tail 200 || true
}
...
while (( SECONDS < ps_deadline )); do
  if "$CLOUDCOMPOSE" compose ps -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" --json | python3 -c "
import json, sys
...
while (( SECONDS < logs_deadline )); do
  if "$CLOUDCOMPOSE" compose logs -f "$COMPOSE_BUILD_COPY" -e "$ENV_DIR" --since 5m --tail 200 --json | python3 -c "
import json, sys
