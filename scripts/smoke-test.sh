...
#
# Creates environment with 'composey init env', compiles a compose file,
...
ENV_DIR="$ROOT/build/$NAME-environment"                # generated by 'composey init env'
...
log "Creating environment '$NAME' with composey init env…"
rm -rf "$ENV_DIR"
cd "$ROOT"
uv run composey init --name "$NAME" --region "$STATE_REGION" --output "$ENV_DIR"
